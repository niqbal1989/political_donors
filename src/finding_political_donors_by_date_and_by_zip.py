    ...
    with open(inf, 'r') as f:
        for line in f:#read in line by line from input
            ...
            if not linelist[15] and (len(linelist[0]) > 0) and (len(linelist[14]) > 0):
                #make sure OTHER_ID is empty, that there is a recipient id and a money contribution
                relevant_list.extend([linelist[0], linelist[10], linelist[13], linelist[14]])
                #create data structure to easily access the only relevant data in the input file 
                if is_zip_valid(relevant_list[1]):#make sure the zipcode is valid
                    ...
                    if zipkey in zipcode_dict:
                        ...
                        #only add information for zipcode keys that already exist in
                        #the dictionary, create new entry if it does not exist
                    median_zip = int(round2(find_median(zipcode_dict[zipkey])))
                    #calculate median thus far for unique identifier and zipcode
                ...
                if is_date_valid(relevant_list[2]):#validate the date in the input file line
                    ...
                    if datekey in date_dict:
                        ...
                        #create dictionary for relevant date data, add to
                        #existing key library only if it already doesn't exist
        id_date = date_dict.keys()
        id_date_data = list(date_dict.values())
        #change from dictionary structure back
        #to list structure for easier manipulation
        for idx, val in enumerate(id_date):
            #for loop to calculate median by date after receiving all the information from the input
            ...
            #compute the median value for the unique recipient and date   
            ...
            #append information to list structure to be written later
        date_list_tbw = sorted(date_list_tbw, key=operator.itemgetter(0, 1))
        for idx, val in enumerate(date_list_tbw):
            #created another for loop because the date data needed to be sorted first
            #and then written out to list by looping over its entries
